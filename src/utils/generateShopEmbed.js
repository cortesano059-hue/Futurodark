...
const eco = require('../database/economy.js');
const ThemedEmbed = require('@src/utils/ThemedEmbed');

// CONFIGURACIÓN
const COLOR_PRINCIPAL = '#2b2d31';
const ITEMS_PER_PAGE = 8;
const IMAGEN_BANNER = 'https://cdn.discordapp.com/attachments/1438575452288581632/1445207801331712214/image.png?ex=693570e6&is=69341f66&hm=28d659750188201993b61af5af33cd1e27583eb58da5470e4e44c181a01a73c8&';
const IMAGEN_FOOTER = 'https://cdn.discordapp.com/attachments/1438575452288581632/1445213527617966201/Tienda_abajo.png?ex=6935763b&is=693424bb&hm=c63621c8f9d0d1315e4fc34e7476a97842b73cba9f6f513bd7ce5d4ac41da1d6&';

/**
 * Genera embeds y componentes para la tienda
 */
async function generateShopPayload(guildId, pageIndex, userId, client) {
    ...
    const itemsOnPage = shopItems.slice(startIndex, startIndex + ITEMS_PER_PAGE);
    ...
    const shopEmbed = new ThemedEmbed()
        .setColor(COLOR_PRINCIPAL)
        .setTitle(`🛍️ Tienda del Servidor (Página ${pageIndex + 1}/${totalPages})`)
        .setDescription(
        ...
        .setFooter({ text: `Mostrando ${startIndex + 1}-${startIndex + itemsOnPage.length} de ${totalItems} artículos.` });

    // --- Menú Desplegable (solo items de la página) ---
    const selectMenuOptions = itemsOnPage.map(item => ({
        label: `${item.itemName} ($${item.price})`,
        description: item.description || 'Comprar este artículo.',
        value: item._id.toString() // identificador único
    }));
...
module.exports = { generateShopPayload };
